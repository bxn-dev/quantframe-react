...
export function OverviewTab({ value }: OverviewTabProps) {
  ...
  const useTranslateTab = (key: string, context?: { [key: string]: any }, i18Key?: boolean) =>
    useTranslateModals(`item_details.tabs.overview.${key}`, { ...context }, i18Key);
  const GetCellValue = (
    cells: {
      i18nKey: string;
      hidden?: boolean;
    ...
    <Group align="flex-start">
      <Group gap="xs" style={{ flex: 1 }} align="flex-start">
        ...
        <Flex direction="column">
          <Group gap={3}>
            <PriceHistoryPopover cooldown={GetProperty("cooldown")} histories={GetProperty("price_history")} status={value.status} size="2x" />
            <ItemName
              hideQuantity
              value={value}
              color="white"
